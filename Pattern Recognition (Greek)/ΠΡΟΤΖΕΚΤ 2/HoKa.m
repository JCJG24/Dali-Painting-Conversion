...
if ( rows(x1) ~= rows(x2) )
	fprintf( 'Error in vectors x1, x2\n' ) ;
end
...
while( i < MaxRep )
  fprintf( 'Step %d\n', i ) ;
  fprintf( 'Classification Score: %7.4f%%\n', ( 100 * NoGreatValMat( b1, 0.0 ) ) / ( rows(b1) * columns(b1)) ) ;
  if ( GreatValMat(b1,0.0) == 1 )
     fprintf( 'Linear Separation of classes in %d repetitions\n', i ) ;
     Rc = Rep ;
     return ;
  end
  ea = AbsMat(e) ;
 ...
 fprintf( 'Original Ho-Kasyap not convergence in %d repetitions\n', i ) ;
 fprintf( 'Classification Score: %7.4f%%\n', ( 100 * NoGreatValMat( b1, 0.0 ) ) / ( rows(b1) * columns(b1)) ) ;
